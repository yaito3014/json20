...
BOOST_AUTO_TEST_CASE(visit)
{
  struct print_visitor {
    ...
    void on_number_unsigned_integer(std::string_view str) noexcept
    {
      std::println("on_number_unsigned_integer: {:?}", str);
    }
    void on_number_signed_integer(std::string_view str) noexcept
    {
      std::println("on_number_signed_integer: {:?}", str);
    }
    void on_number_floating_point(std::string_view str) noexcept
    {
      std::println("on_number_floating_point: {:?}", str);
    }
...

  {
    ...
    const auto json = json_visitor.get();
  }
  {
    yk::json20::basic_json_visitor<char> json_visitor;
    json_visitor.on_object_start();                  // prepare map
